<x-app-layout>
    <div class="bg-blackBackground text-white min-h-screen flex flex-col font-fredoka">
        ...
        <section class="relative bg-cover bg-center min-h-[90vh] flex justify-center items-center"
            ...
            <div class="z-10 text-center px-6">
                ...
                <p class="mt-4 text-2xl md:text-3xl font-bold text-white font-roboto">
                    {{ __('messages.update_your_reservation') }}
                </p>
            </div>
        </section>

        <!-- Reservation Edit Form Section -->
        <div class="py-16 px-6 max-w-3xl mx-auto">
            ...
            <form action="{{ route('reserves.update', $reservation->id) }}" method="POST"
                ...
                <div>
                    ...
                    <input type="date" id="date" name="date"
                        value="{{ $reservationDate }}" required
                ...
                <div>
                    ...
                    <input type="time" id="time" name="time"
                        value="{{ $reservationTime }}" required
                        class="mt-1 p-3 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-gold focus:ring-primary-gold bg-blackShader text-white">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="num_guests" name="num_guests" value="{{ $reservation->num_guests }}"
                        required min="1"
                        class="mt-1 p-3 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-gold focus:ring-primary-gold bg-blackShader text-white">
                </div>
                ...
                <div>
                    ...
                    <select id="taula_id" name="taula_id" required
                        ...
                        @foreach ($tables as $table)
                            <option value="{{ $table->id }}" @if ($table->id == $reservation->taula_id) selected @endif>
                                Table #{{ $table->id }} ({{ $table->capacitat }} guests)
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <input type="hidden" name="estat" value="{{ $reservation->estat }}">
